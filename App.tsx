...
const TRANSLATIONS: Record<LanguageCode, any> = {
  es: {
    ...
    alertImageSol: "Por favor sube una imagen de tu intento.",
  ...
  en: {
    ...
    alertImageSol: "Please upload an image of your attempt.",
  ...
  pt: {
    ...
    alertImageSol: "Por favor, envie uma imagem da sua tentativa.",
  ...
  fr: {
    ...
    alertImageSol: "Veuillez télécharger une image de votre tentative.",
  ...
  de: {
    ...
    alertImageSol: "Bitte laden Sie ein Bild Ihres Versuchs hoch.",
  ...
  ja: {
    ...
    alertImageSol: "試行の画像をアップロードしてください。",
  ...
  ru: {
    ...
    alertImageSol: "Загрузите изображение попытки.",
  ...
  ar: {
    ...
    alertImageSol: "يرجى تحميل صورة محاولتك.",
  ...
  hi: {
    ...
    alertImageSol: "कृपया अपने प्रयास की एक छवि अपलोड करें।",
...
const getBrowserLanguage = (): LanguageCode => {
    ...
    return supported ? (lang as LanguageCode) : 'es';
};
...
const App: React.FC = () => {
  ...
  const [language, setLanguage] = useState<LanguageCode>(getBrowserLanguage());
  ...
  const validateInput = (): boolean => {
    ...
    if (formData.solutionAttemptType === 'image' && !formData.solutionImage) {
      alert(t('alertImageSol'));
      return false;
    }
  ...
  const startAnalysis = async () => {
    ...
    try {
      // NON-BLOCKING: Fire video summarization in background if needed
      if (formData.solutionAttemptType === 'video' && formData.solutionVideo) {
          GeminiService.summarizeMultimedia(
              formData.solutionVideo, 
              formData.solutionVideo.type,
              t('videoSummaryPrompt')
          ).then(summary => {
              if (summary) setFormData(prev => ({ ...prev, generatedVideoSummary: summary }));
          });
      }
      ...
      else if (formData.solutionAttemptType === 'image') solutionContent = formData.solutionImage!;
      ...
      // Get the stream
      const stream = await GeminiService.startDiagnosis(
      ...
      setChatHistory(prev => [
        ...
        { id: Date.now().toString(), role: 'model', text: '' } // Empty message to append to
      ]);

      let fullText = '';
      for await (const chunk of stream) {
        const text = chunk.text; // Use property, not method
        if (text) {
            fullText += text;
            setChatHistory(prev => {
                const lastMsg = prev[prev.length - 1];
                const others = prev.slice(0, -1);
                return [...others, { ...lastMsg, text: fullText }];
            });
        }
      }
  ...
  const handleSendMessage = async () => {
    ...
    const newMsg: ChatMessage = { 
        ...
        text: userInput,
        image: imageUrl,
        file: chatImage || undefined // Store the file reference for later upload
    };
    
    setChatHistory(prev => [...prev, newMsg, { id: 'thinking', role: 'model', text: t('thinking'), isThinking: true }]);
    setUserInput('');
    setChatImage(null);

    try {
      const stream = await GeminiService.sendChatMessage(newMsg.text, chatImage || undefined);
      
      // Remove thinking, add empty model message
      setChatHistory(prev => [
        ...prev.filter(m => !m.isThinking),
        { id: Date.now().toString(), role: 'model', text: '' }
      ]);
        ...
        <div className="p-6 border-b border-gray-100 dark:border-gray-700">
          <div className="flex justify-between items-center mb-2">
            ...
            <div className="flex bg-gray-100 dark:bg-gray-900 rounded-lg p-1">
                ...
                >{t('textBtn')}</button>
                <button 
                    onClick={() => handleInputChange('solutionAttemptType', 'image')}
                    className={`px-3 py-1 text-xs rounded-md font-medium transition-all ${formData.solutionAttemptType === 'image' ? 'bg-white dark:bg-gray-700 shadow text-blue-600 dark:text-blue-400' : 'text-gray-500'}`}
                >{t('imageBtn')}</button>
                <button 
          ...
          {formData.solutionAttemptType === 'image' && (
             <div className="flex items-center justify-center w-full mt-2">
                 <label className={`flex flex-col items-center justify-center w-full ${formData.solutionImage ? 'h-48' : 'h-32'} border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:hover:bg-gray-700 dark:bg-gray-900 hover:bg-gray-100 dark:border-gray-600 dark:hover:border-gray-500 transition-all duration-300 relative overflow-hidden`}>
                     {formData.solutionImage ? (
                         <>
                             ...
                             />
                             <div className="absolute inset-0 bg-black/50 flex flex-col items-center justify-center opacity-0 hover:opacity-100 transition-opacity z-20">
                                 <p className="text-white font-medium text-xs">{t('clickUpload')}</p>
                             </div>
                         </>
                     ) : (
                         <div className="flex flex-col items-center justify-center pt-5 pb-6">
                             <svg className="w-6 h-6 mb-2 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path></svg>
                             <p className="text-xs text-gray-500 dark:text-gray-400">{t('clickUpload')}</p>
                         </div>
                     )}
                     <input type="file" className="hidden" accept="image/*" onChange={(e) => handleInputChange('solutionImage', e.target.files?.[0] || null)} />
                 </label>
             </div>
          )}
                                ...
                                <input 
                                    ...
                                    onChange={(e) => {
                                        if (e.target.files?.[0]) {
                                            ...
                                            e.target.value = ''; // Reset value to allow re-upload
                                        }
                                    }}
                                />
                ...
                {historySessions.length === 0 && !isLoadingHistory ? (
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {historySessions.map(session => (
                            ...
                            >
                                <div className="h-64 bg-gray-200 dark:bg-gray-700 relative overflow-hidden flex items-center justify-center">
                                    {session.fileUrls?.problemImage ? (
                                        <>
                                            <div 
                                                className="absolute inset-0 bg-cover bg-center blur-md opacity-50 scale-110"
                                                style={{ backgroundImage: `url(${session.fileUrls.problemImage})` }}
                                            />
                                            <img 
                ...
                <div className="flex justify-between h-16 items-center">
                    ...
                    <div className="flex items-center gap-4">
                        <button 
                            ...
                            className={`flex items-center gap-2 px-5 py-2.5 rounded-full font-bold transition-all duration-300 shadow-md ${
                                viewMode === 'history'
                                ? 'bg-gradient-to-r from-violet-600 to-indigo-600 text-white ring-2 ring-violet-300 hover:scale-105'
                                : 'bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-200 border-2 border-indigo-50 dark:border-indigo-900 hover:border-indigo-300 hover:bg-gray-50 dark:hover:bg-gray-700'
                            }`}
                            title={t('historyBtn')}
                        >
                             <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2v-2z"></path></svg>
                            <span className="hidden sm:inline text-sm">{t('historyBtn')}</span>
                        </button>
                        ...
                        <div className="relative">
                            <select 
                                ...
                                className="appearance-none pl-4 pr-10 py-2 rounded-lg border-2 border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 text-sm font-semibold text-gray-700 dark:text-gray-200 hover:border-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all cursor-pointer shadow-sm"
                            >
                                {LANGUAGES.map(lang => (
